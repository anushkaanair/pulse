...
export default function WatchlistPage() {
  ...
  const [conflict, setConflict] = useState<{ theirs: WatchlistItem[]; mine: string[] }>();
  ...
  const saveBulk = async (symbols = requestedSymbols()) => { try { setWatchlist(await api.replaceItems(id, symbols, watchlist!.version)); setEditing(false); setConflict(undefined); } catch (cause) { const current = cause instanceof ApiRequestError ? (cause.response as unknown as Partial<ConflictResponse>).current : undefined; if (current) setConflict({ theirs: current.items, mine: symbols }); else setError(cause); } };
  ...
  </div>{conflict ? <ConflictModal theirs={conflict.theirs} mine={conflict.mine} onKeepMine={() => void saveBulk(conflict.mine)} onKeepTheirs={() => { setWatchlist((current) => current ? { ...current, version: current.version + 1, items: conflict.theirs } : current); setConflict(undefined); setEditing(false); }} onMerge={() => void saveBulk([...new Set([...conflict.theirs.map((item) => item.symbol), ...conflict.mine])])} /> : null}</main>;
}
